...
- Zod, Better Auth, Busboy streaming uploads, Google APIs client
...
- `RESEND_API_KEY`, `RESEND_FROM_EMAIL` (password reset OTP emails via Resend, e.g. `Archive Cloud <onboarding@resend.dev>`)
- `CRON_SECRET` (optional; min 16 chars. Enables `GET|POST /cron/tick` for scheduled transfers + folder sync polling. Send `Authorization: Bearer <CRON_SECRET>` or `x-cron-secret`)
- `GITHUB_TOKEN` (optional; classic PAT or fine-grained token with public repo read. Raises GitHub API rate limits for navbar star counts; without it unauthenticated limit is 60 req/hour/IP)
- `WEBHOOK_BASE_URL` (optional HTTPS public URL for provider webhooks; defaults to `APP_URL`. Google Drive `changes.watch` requires HTTPS — set this to your production or ngrok URL)
- `POLAR_ACCESS_TOKEN` (Polar org access token; enables checkout)
- `POLAR_WEBHOOK_SECRET` (Polar webhook signing secret)
- `POLAR_SERVER` (`sandbox` or `production`, default `sandbox`)
- `POLAR_PRODUCT_THUNDER_LIFETIME` (Polar product UUID for Thunder — $9 one-time lifetime; legacy alias `POLAR_PRODUCT_POWER_LIFETIME` still works)
- `ADMIN_EMAIL` (optional; comma-separated instance admin emails — treated as Thunder with no plan restrictions; Typebot-style self-host admin)
- `ADMIN_NAME` (optional; display name applied to the admin account on signup)
- `DEFAULT_USER_PLAN` (optional; `free` or `thunder` — plan stored for new non-admin signups on this instance; default `free`; legacy `power` accepted)
- `BILLING_ENABLED` (optional; when `false`, hides checkout UI and blocks `POST /billing`; defaults to `true` when Polar is configured, else `false`)

Docker (self-host, Typebot-style):

- `Dockerfile` (standalone Next.js + entrypoint runs `prisma migrate deploy`)
- `docker-compose.prod.yml` — app + Postgres, `env_file: .env`
- `docker-compose.dev.yml` — Postgres only for local `pnpm dev`
- `.env.example` — starter env (`DATABASE_URL` points at `archivecloud-db`)
- `.dockerignore` excludes `**/.env` (secrets only via Compose `env_file`)
- Self-host: `cp .env.example .env` → fill secrets → `docker compose -f docker-compose.prod.yml up --build -d`
...
- `GET|POST /cron/tick` (public; `CRON_SECRET`) — scheduled transfers, folder sync poll, Google watch renewals
- `POST /webhooks/google-drive` (public) — Google Drive / Shared Drive push notifications
- `GET|POST /webhooks/onedrive` (public) — Microsoft Graph subscription validation + OneDrive change notifications
- `GET|POST /webhooks/dropbox` (public) — Dropbox webhook challenge + change notifications
- `POST /connected-accounts/:id/webhooks/enable` — register/renew watches for Google Drive/OneDrive/Dropbox
- `GET|POST /api-keys`, `DELETE /api-keys/:id` — manage user API keys (session auth)
- `GET /api/v1/accounts`, `GET|POST /api/v1/transfers` — thin API (Bearer `ack_…` API key or session)
