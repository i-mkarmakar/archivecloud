...
> = {
  ...
  md: {
    ...
    date: "mt-2 truncate text-xs text-muted",
    tagsShell:
      "mt-3 flex flex-wrap justify-center gap-2 text-xs font-semibold text-muted",
    tag: "rounded-full bg-surface-secondary px-2.5 py-1",
    mtCard: "mt-5",
  },
  lg: {
    ...
    title:
      "line-clamp-2 min-h-12 text-base font-extrabold text-foreground mt-6 sm:text-lg",
    date: "mt-2 truncate text-sm text-muted",
    tagsShell:
      "mt-4 flex flex-wrap justify-center gap-2 text-sm font-semibold text-muted",
    tag: "rounded-full bg-surface-secondary px-3 py-1.5",
    mtCard: "mt-6",
  },
};
        ...
        return (
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <input
                type="checkbox"
                className={cn("shrink-0 accent-accent", cfg.checkbox)}
                checked={selected}
              ...
              <button
                type="button"
                className={cn(
                  "flex shrink-0 items-center justify-center rounded-xl text-muted hover:bg-white/80",
                  cfg.menuBtn,
                )}
                onClick={(event) => {
            ...
            <div className={cn("min-w-0 text-center", cfg.mtCard)}>
              <h3
                className={cn(
                  "font-extrabold text-foreground line-clamp-2",
                  cfg.title,
                )}
                title={file.name}
              >
              ...
              <div
                className={cn(
                  "flex flex-wrap justify-center font-semibold text-muted",
                  cfg.tagsShell,
                )}
              >
                <span
                  className={cn("rounded-full bg-surface-secondary", cfg.tag)}
                >
                  {file.size}
                </span>
                <span
                  className={cn(
                    "max-w-full truncate rounded-full bg-surface-secondary",
                    cfg.tag,
                  )}
                >
                  {file.access}
