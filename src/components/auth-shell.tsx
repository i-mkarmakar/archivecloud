...
export function AuthShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setBgReady(true), 2500);
    return () => window.clearTimeout(timer);
  }, []);

  return (
    <>
      {!bgReady ? <AppPreloader /> : null}
      <div
        aria-hidden={!bgReady}
        className={
          bgReady
            ? "grid min-h-svh lg:grid-cols-[40%_60%]"
            : "pointer-events-none invisible grid min-h-svh lg:grid-cols-[40%_60%]"
        }
      >
